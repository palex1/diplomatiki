...
# 1. Διαμόρφωση & Setup
# ----------------------------------------------------
st.set_page_config(page_title="Εκπαιδευτικός Σχεδιαστής", layout="wide")
...
# 3. Κύριο Μέρος
# ----------------------------------------------------
st.title("🎓 AI Σχεδιαστής Εκπαιδευτικού Υλικού")
st.markdown(f"Δημιουργία υλικού για: **{subject_area}** ({grade})")

# Ορίζουμε το μοντέλο σε 1.5 flash που είναι πιο σταθερό στο free tier
...
if submit_btn and client:
    ...
    # -------------------------------------------
    with tab1:
        st.subheader("Αναλυτικό Διδακτικό Σενάριο")
        with st.spinner("Σχεδιασμός σεναρίου..."):
            prompt_scenario = (
                ...
                f"Μέθοδος: {method}. "
                "Παρουσίασε τη ροή του μαθήματος σε μορφή πίνακα Markdown."
            )
            try:
    ...
    with tab3:
        ...
        with st.spinner("Σύνταξη ερωτήσεων... (Περιμένετε λίγο)"):
            prompt_quiz = (
                f"Φτιάξε ένα Quiz 5 ερωτήσεων πολλαπλής επιλογής για το '{subject_area}'. "
                "Στο τέλος βάλε τις λύσεις."
            )
            try:
